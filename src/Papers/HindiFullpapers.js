...
function HindiFullPapers() {
  ...
  const API_hisory_paper =
    "http://13.127.101.77/api/v1/get-question?subject=hindi&medium=english&standard=10 ";
  ...
  const fetchdata = async (url) => {
    try {
      ...
      await sleep(3000);
      setData(data.data);
      localStorage.setItem("data_hindi_full", JSON.stringify(data));
  ...
  useEffect(() => {
    ...
    const timestamp = localStorage.getItem('timestamp_hindi_full');
    const data_hindi_full = localStorage.getItem('data_hindi_full');
    ...
    if (timestamp && data_hindi_full) {
      ...
      if (check) {
        ...
        localStorage.removeItem('timestamp_hindi_full');
        localStorage.removeItem('data_hindi_full');
        ...
        localStorage.setItem('timestamp_hindi_full', JSON.stringify({
          value: "string",
          expDate: date,
        }))

        fetchdata(API_hisory_paper);
      } else if(localStorage.getItem('data_hindi_full')) {
        const object = JSON.parse(localStorage.getItem('data_hindi_full'))
        setData(object.data)
        document.getElementById("loader").classList.add("hidden");
        document.getElementById("parent").classList.remove("hidden");
        document.getElementById("explore").classList.remove("hidden");
