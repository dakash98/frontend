...
function Math2Papers() {
  ...
  const API_hisory_paper =
    "http://13.127.101.77/api/v1/get-question?subject=math_2&medium=english&standard=10 ";
  ...
  const fetchdata = async (url) => {
    try {
      ...
      await sleep(3000);
      setData(data.data);
      localStorage.setItem("data_math_2", JSON.stringify(data));
  ...
  useEffect(() => {
    ...
    if (timestamp &&  data_math_2) {
      ...
      if (check) {
        ...
        localStorage.setItem('timestamp_math_2', JSON.stringify({
          value: "string",
          expDate: date,
        }))

        fetchdata(API_hisory_paper);
      } else if(localStorage.getItem('data_math_2')) {
        const object = JSON.parse(localStorage.getItem('data_math_2'))
        setData(object.data)
        document.getElementById("loader").classList.add("hidden");
        document.getElementById("parent").classList.remove("hidden");
        document.getElementById("explore").classList.remove("hidden");
