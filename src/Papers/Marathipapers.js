...
function MarathiPapers() {
  ...
  const API_hisory_paper =
    "http://13.127.101.77/api/v1/get-question?subject=marathi&medium=english&standard=10 ";
  ...
  const fetchdata = async (url) => {
    try {
      ...
      await sleep(3000);
      setData(data.data);
      localStorage.setItem("data_marathi", JSON.stringify(data));
  ...
  useEffect(() => {
    ...
    if (timestamp && data_marathi) {
      ...
      if (check) {
        ...
        localStorage.setItem('timestamp_marathi', JSON.stringify({
          value: "string",
          expDate: date,
        }))

        fetchdata(API_hisory_paper);
      } else if(localStorage.getItem('data_marathi')) {
        const object = JSON.parse(localStorage.getItem('data_marathi'))
        setData(object.data)
        document.getElementById("loader").classList.add("hidden");
        document.getElementById("parent").classList.remove("hidden");
        document.getElementById("explore").classList.remove("hidden");
