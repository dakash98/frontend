...
function Math1Papers() {
  ...
  const fetchdata = async (url) => {
    try {
      ...
      await sleep(3000);
      setData(data.data);
      localStorage.setItem("data_math_1", JSON.stringify(data));
  ...
  useEffect(() => {
    ...
    const timestamp = localStorage.getItem('timestamp_math_1');
    const data_math_1 = localStorage.getItem('data_math_1');
    ...
    if (timestamp && data_math_1) {
      ...
      if (check) {
        ...
        localStorage.removeItem('timestamp_math_1');
        localStorage.removeItem('data_math_1');
        ...
        localStorage.setItem('timestamp_math_1', JSON.stringify({
          value: "string",
          expDate: date,
        }))

        fetchdata(API_hisory_paper);
      } else if(localStorage.getItem('data_math_1')) {
        const object = JSON.parse(localStorage.getItem('data_math_1'))
        setData(object.data)
        document.getElementById("loader").classList.add("hidden");
        document.getElementById("parent").classList.remove("hidden");
        document.getElementById("explore").classList.remove("hidden");
