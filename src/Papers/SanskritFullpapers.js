...
function SanskritFullPapers() {
  ...
  const API_hisory_paper =
    "http://13.127.101.77/api/v1/get-question?subject=sanskrit_full&medium=english&standard=10 ";
  ...
  const fetchdata = async (url) => {
    try {
      ...
      await sleep(3000);
      setData(data.data);
      localStorage.setItem("data_sanskrit_full", JSON.stringify(data));
  ...
  useEffect(() => {
    ...
    if (timestamp && data_sanskrit_full) {
      ...
      if (check) {
        ...
        localStorage.setItem('timestamp_sanskrit_full', JSON.stringify({
          value: "string",
          expDate: date,
        }))

        fetchdata(API_hisory_paper);
      } else if(localStorage.getItem('data_sanskrit_full')) {
        const object = JSON.parse(localStorage.getItem('data_sanskrit_full'))
        setData(object.data)
        document.getElementById("loader").classList.add("hidden");
        document.getElementById("parent").classList.remove("hidden");
        document.getElementById("explore").classList.remove("hidden");
